...
class DatasetLoader:
    ...
    def load_data(self, train_file=None, test_file=None, normalize=True, one_hot=True, num_classes=10):
        ...
        if self.dataset_type == "mnist":
            ...
            return self._load_custom_dataset(self.custom_path, normalize, one_hot, num_classes)
        else:
    ...
    def _load_custom_dataset(self, dataset_path, normalize, one_hot, num_classes):
        """Loads a custom dataset from CSV or NumPy format."""
        if dataset_path.endswith(".csv"):
            print("Loading custom CSV dataset...")
            data = pd.read_csv(dataset_path, header=None).values  # Ensure header is not misread
            X, y = data[:, 1:], data[:, 0]  # Assume first column is labels
    ...
    def _prepare_data(self, X_train, y_train, X_test, y_test, normalize, one_hot, num_classes):
        """Prepares dataset (normalization, one-hot encoding, splitting validation)."""
        if normalize:
            X_train = X_train.astype("float32") / 255.0
            X_test = X_test.astype("float32") / 255.0
